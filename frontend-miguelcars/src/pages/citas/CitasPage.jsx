...
export default function CitasPage() {
  ...
  const load = () => {
    ...
    Promise.all([getCitas(), getClientes(), getVehiculos(), getUsuarios()])
      .then(([ci, cl, v, u]) => {
        setData(ci.data); setClientes(cl.data);
  ...
  const openEdit = row => {
    setForm({
      ...
      motivo: row.motivo || '', estado: row.estado || 'PROGRAMADA',
  ...
  const buildPayload = () => ({
    ...
    motivo: form.motivo, estado: form.estado,
  ...
  const handleDelete = async row => {
    if (confirm(`¿Eliminar cita #${row.id}?`)) { await deleteCita(row.id); load(); }
  };
  ...
  return (
    <div style={{ animation:'fadeIn .3s ease' }}>
      <PageHeader title="Citas" onAdd={openNew} addLabel="Nueva Cita" />
        ...
        <Modal title={editing ? 'Editar Cita' : 'Nueva Cita'} onClose={close}>
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
              <FormField label="Fecha" type="date" value={form.fecha || ''} onChange={set('fecha')} required />
              <FormField label="Hora"  type="time" value={form.hora  || ''} onChange={set('hora')}  required />
            </div>
